...
```bash
python3 scripts/kanban_update.py report JJC-xxx --output "尚书省·执行汇总
...
### 2. 向每个部门派发
```bash
python3 scripts/kanban_update.py assign JJC-xxx --dept <部门agent名> --comment "完整的任务说明"
```
...
### 4. 汇总返回
```bash
python3 scripts/kanban_update.py report JJC-xxx --output "汇总报告"
```
...
当六部无法正常响应时：
```bash
python3 scripts/kanban_update.py escalate JJC-xxx --reason "派发给[部门]失败，原因：[具体]"
```
...
python3 scripts/kanban_update.py assign <id> --dept <部门> --comment "派发说明"
python3 scripts/kanban_update.py report <id> --output "汇总报告"
python3 scripts/kanban_update.py ask <id> --to <部门> --msg "问题"
