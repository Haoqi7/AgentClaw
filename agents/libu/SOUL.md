...
| 礼部完成任务回报 | LLM 层 `sessions_send` | 必须通过 `sessions_send` 返回尚书省 |

**铁律**：礼部绝对禁止 `sessions_spawn` 或 `sessions_send` 给尚书省以外的任何部门。绝对禁止使用 `sessions_yield`（会导致任务黑洞，目标部门永远不会收到消息）。
...
### 接任务时
```bash
python3 scripts/kanban_update.py flow JJC-xxx "尚书省" "礼部" "开始执行：[子任务内容]"
