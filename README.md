...
**不是 prompt 套 prompt 的简单链式调用，而是一套有编制、有流程、有监督、可回滚的真实组织体系。**【有关键词才走三省六部通道】
...
### 📮 发送你的第一道旨意
**派发关键词清单：**  
`接旨` `派发` `交给中书省` `传旨` `下旨` `动用六部`  必须有这些关键词才会派发任务。  

通过消息渠道发送：

```
接旨：请帮我用 Python 写一个文本分类器：
1. 使用 scikit-learn
2. 支持多分类
3. 输出混淆矩阵
4. 写完整的文档
...
docker run -d \
  ...
  -v /path/to/agentclaw-data:/app/AgentClaw/data \  
 haoqi7/openclaw:latest
```
⚠️ 必须挂载两个目录，否则更新镜像后任务数据会丢失：
...
| 挂载卷 | `/app/AgentClaw/data`看板任务数据（tasks_source.json、live_status.json 等）|
